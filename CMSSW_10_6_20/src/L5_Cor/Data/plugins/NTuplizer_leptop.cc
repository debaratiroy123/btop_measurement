...
private:
  ...
  float pfjetAK8muinpt[njetmxAK8], pfjetAK8muineta[njetmxAK8], pfjetAK8muinphi[njetmxAK8], pfjetAK8muine[njetmxAK8], pfjetAK8muinmass[njetmxAK8];

  float pfjetAK8elinpt[njetmxAK8], pfjetAK8elineta[njetmxAK8], pfjetAK8elinphi[njetmxAK8], pfjetAK8eline[njetmxAK8], pfjetAK8elinmass[njetmxAK8];
  ...
  float muonchiso[njetmx], muonnhiso[njetmx], muonphiso[njetmx], muonminisoall[njetmx]; //muonpuiso[njetmx];
  ...
  const char *hlt_name[nHLTmx] = {"HLT_IsoMu24_v","HLT_Mu50_v","HLT_Ele50_CaloIdVT_GsfTrkIdT_PFJet165_v","HLT_Ele115_CaloIdVT_GsfTrkIdT_v", "HLT_AK8PFJet500_v", "HLT_Ele32_WPTight_Gsf_v", "HLT_Photon200_v", "HLT_Ele27_Ele37_CaloIdL_MW_v", "HLT_Mu37_Ele27_CaloIdL_MW_v", "HLT_Mu27_Ele37_CaloIdL_MW_v", "HLT_Mu37_TkMu27_v", "HLT_TkMu100_v", "HLT_DoubleEle25_CaloIdL_MW_v"};

  //HLT_AK8PFJet360_TrimMass30_v = > can be added 
  //HLT_Ele20_WPLoose_Gsf_v : this was there till 19th Jan, 2020 as 6th element 

  int ihlt01, ihlt02, ihlt03, ihlt04, ihlt05, ihlt06, ihlt07, ihlt08, ihlt09, ihlt10, ihlt11, ihlt12, ihlt13;
  float prescl01, prescl02, prescl03, prescl04, prescl05, prescl06, prescl07, prescl08, prescl09, prescl10, prescl11, prescl12, prescl13;
  ...
  hltPrescaleProvider_(pset, consumesCollector(), *this)
{
  ...
  T1->Branch("ihlt11",&ihlt11,"ihlt11/I");
  T1->Branch("ihlt12",&ihlt12,"ihlt12/I");
  T1->Branch("ihlt13",&ihlt13,"ihlt13/I");
  
  /*
    T1->Branch("ihlt14",&ihlt14,"ihlt14/I");
    T1->Branch("ihlt15",&ihlt15,"ihlt15/I");
    T1->Branch("ihlt16",&ihlt16,"ihlt16/I");
    T1->Branch("ihlt17",&ihlt17,"ihlt17/I");
  */
  ...
  T1->Branch("prescl11",&prescl11,"prescl11/F");
  T1->Branch("prescl12",&prescl12,"prescl12/F");
  T1->Branch("prescl13",&prescl13,"prescl13/F");
  /*
  T1->Branch("prescl14",&prescl14,"prescl14/F");
  T1->Branch("prescl15",&prescl15,"prescl15/F");
  T1->Branch("prescl16",&prescl16,"prescl16/F");
  T1->Branch("prescl17",&prescl17,"prescl17/F");
  */
...
//  T1->Branch("pfjetAK4partonpdg",pfjetAK4partonpdg,"pfjetAK4partonpdg[npfjetAK4]/I");
  ...
  //T1->Branch("muonpuiso", muonpuiso, "muonpuiso[nmuons]/F");
...
Leptop::analyze(const edm::Event& iEvent, const edm::EventSetup& pset) {
  ...
  if(pfjetAK8s.isValid()){
	...
	if (abs((*daught[i3]).pdgId()) == 11){
	  if ((*daught[i3]).pt() > el_maxpt) {
	    ...
	    el_indx = i3;
	    //std::cout << " el_maxpt " << el_maxpt << " el_indx " << el_indx << std::endl; 
	  }
	}
      }
      if (el_indx >= 0) {
	//std::cout << (*daught[el_indx]).pt() << (*daught[el_indx]).eta() << (*daught[el_indx]).phi() << (*daught[el_indx]).energy() << " ievt " << ievt << " for largejet " << npfjetAK8 << std::endl;
	...
	if(((*pfjetAK8s)[jet].subjets(subjets)).size()>1){   ////subjets = "SoftDropPuppi"
	  ...
	  for(unsigned int isub=0; isub<(((*pfjetAK8s)[jet].subjets(subjets)).size()); isub++){
		...
		elInsubjet2vec.push_back(elInsubjet2);
	      ...
	      if (mu_indx >= 0 && isub==0) {
                //std::cout << (*subdaught[mu_indx]).pt() << (*subdaught[mu_indx]).eta() << (*subdaught[mu_indx]).phi() << (*subdaught[mu_indx]).energy() << " isub " << isub <<" ievt " << ievt << " for largejet " << jet << std::endl;                             
	  ...
	  if (pfjetAK8sub1hadfrac[npfjetAK8] > 0 && pfjetAK8sub2hadfrac[npfjetAK8] > 0) {
		...
		//std::cout << " pfjetAK8elinsubpt[npfjetAK8] " <<  npfjetAK8  << " " << pfjetAK8elinsubpt[npfjetAK8] << " isub " << 0 << std::endl;
		//" " << pfjetAK8elinsubeta[npfjetAK8] << " " << pfjetAK8elinsubphi[npfjetAK8] << " " << pfjetAK8elinsube[npfjetAK8] << " " << pfjetAK8elinsubmass[npfjetAK8] << " " <<  "pfjetAK8elinsubjpt[npfjetAK8]" <<  " " << pfjetAK8elinsubjpt[npfjetAK8] << " " << pfjetAK8elinsubjeta[npfjetAK8] << " " << pfjetAK8elinsubjphi[npfjetAK8] << " " << pfjetAK8elinsubjmass[npfjetAK8] << " " << pfjetAK8elinsubje[npfjetAK8] << std::endl;
		...
		//std::cout << " pfjetAK8elinsubpt[npfjetAK8] " <<  npfjetAK8  << " " << pfjetAK8elinsubpt[npfjetAK8] << " isub " << 1 << std::endl;
	  ...
	  if (pfjetAK8elinsubpt[npfjetAK8] != -100 && pfjetAK8elinsubeta[npfjetAK8] != -100) {
	    ...
	    //std::cout << " el Ifar " << elIfar << " el Inear " << elInear << " el I0 " << elI0 << " el Ifar/I0 " << elIfar/elI0 << " el Inear/I0 " << elInear/elI0 << std::endl;
	  ...
	  if (pfjetAK8muinsubpt[npfjetAK8] != -100 && pfjetAK8muinsubeta[npfjetAK8] != -100) {
	    ...
	    for (unsigned int i = 0, n = pfs->size(); i < n; ++i) {
	      ...
	      //std::cout << " Inear " << Inear << " pf.pt() " << pf.pt() << " pow(dR,-2.0) " << pow(dR,-2.0) << " dR " << dR << " pfjetAK8muinsubeta[npfjetAK8] " << pfjetAK8muinsubeta[npfjetAK8] << " pfjetAK8muinsubphi[npfjetAK8] " << pfjetAK8muinsubphi[npfjetAK8] << " pf.eta() " << pf.eta() << " pf.phi() " << pf.phi() << std::endl;
	      I0 = I0 + pf.pt();
	      }
	    }
	    //std::cout << " mu Ifar " << Ifar << " mu Inear" << Inear << " mu I0 " << I0 << " muIfar/muI0 " << Ifar/I0 << " muInear/muI0 " << Inear/I0 << std::endl;
  ...
  */
  ...
  if(muons.isValid() && muons->size()>0) {
	...
	//std::cout << muon1->charge() << std::endl;
  ...
  for(const auto& electron1 : iEvent.get(tok_electrons_) ) {
    ...
    */
    ...
    //if (electron1.pt() < 0.) std::cout << electron1.pt() << " " << std::endl;
    ...
    //std::cout << eldxy[nelecs] << " " << eldxytrk[nelecs] << " " << eldz[nelecs] << " " << eldztrk[nelecs] << " " << gsftrk1->dz() << " " << std::endl; 
  ...
  for(int jk=0; jk<nHLTmx; jk++) {
    ...
    case 12 :
      ...
      /*  
	  case 13 :
	  //std::cout << " trigger passed " << ihlttrg[jk] << std::endl; 
	  ihlt14 = ihlttrg[jk];
