...
Bool_t Anal_Leptop_PROOF::Process(Long64_t entry)
{
  ...
  for(int ijet=0; ijet<npfjetAK8; ijet++){
    ...
    //JetID = (abs(eta)<=2.6 && CEMF(Charged EM Fraction)<0.8 && CHM(Charged Multiplicity)>0 && CHF(Charged Hadron Fraction)>0 && NumConst>1 && NEMF(Neutral EM Fraction)<0.9 && MUF(Muon Fraction)<0.8 && NHF(Neutral Hadron Fraction) < 0.9 ); //latest JETID recommendation : https://twiki.cern.ch/twiki/bin/view/CMS/JetID13TeVRun2018
    
    //Contruct Tight Jet ID without lepton veto components//
    
    int NumConst = pfjetAK8Chcons[ijet] + pfjetAK8Neucons[ijet];
    bool TightLJetID = false;
    
    if (fabs(pfjetAK8y[ijet])<=2.6 && pfjetAK8CHM[ijet]>0 && pfjetAK8CHF[ijet]>0 && NumConst>1 && pfjetAK8NEMF[ijet]<0.9 && pfjetAK8NHF[ijet]<0.9) TightLJetID = true;
    
    if (TightLJetID == false) continue;
  ...
  //TString str1,str2,str3,str4;
  //str1 = TString::Format("event id without lepton selection %u", ievt);
  //if(gProofServ) gProofServ->SendAsynMessage(str1);
  
  //if (emu_ch == 1) str2 = TString::Format("event emu %u",ievt);
  //if(gProofServ) gProofServ->SendAsynMessage(str2);
  
  //if (mumu_ch == 1) str3 = TString::Format("event mumu %u",ievt);
  //if(gProofServ) gProofServ->SendAsynMessage(str3);

  //if (ee_ch == 1) str4 = TString::Format("event ee %u",ievt);
  //if(gProofServ) gProofServ->SendAsynMessage(str4);
  ...
  //# of triggers as on May end 2021 which now is modified in .cc, have to modify accordingly
  ...
  //ihlt09=HLT_Mu37_Ele27_CaloIdL_MW_v, ihlt10=HLT_Mu27_Ele37_CaloIdL_MW_v, ihlt11=HLT_Mu37_TkMu27_v, ihlt13=HLT_DoubleEle25_CaloIdL_MW_v
  
  itrig_pass = ((ihlt09==1)||(ihlt10==1)||(ihlt11==1)||(ihlt13==1));
...
#ifdef E_MU_TTBar
  itrigemu_pass = ((ihlt09==1)||(ihlt10==1));
  if(!itrigemu_pass) return kFALSE;
#elif defined(E_E_TTBar)
  itrigee_pass = (ihlt13==1);
  if(!itrigee_pass) return kFALSE;
#elif defined(MU_MU_TTBar)
  itrigmumu_pass = (ihlt11==1);
  ...
  /*trigger checks for EMu final state*/
  ...
  if (fsttrig==1 || sndtrig==1) {if(!(fmucand.Pt()>mulpt && felcand.Pt()>ellpt)) return kFALSE;}
  ...
  //TString str;
  //str = TString::Format("event id fmucand.Pt() %f felcand.Pt() %f", fmucand.Pt(),felcand.Pt());
